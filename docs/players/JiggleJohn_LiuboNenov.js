...
onGameMessage(({ ownShip, targets }) => {
	...
	if (target.range < 150) {
		...
		switch (target.bowSector) {
			case 1:
				if (target.bearingSector < 6) fireSector = target.bearingSector;
				else fireSector = target.bearingSector + 0.5;
				break;
			case 2:
				if (target.bearingSector < 6) fireSector = target.bearingSector;
				else fireSector = target.bearingSector + 0.5;
	...
	if (ownShip.blockedSector) {
		speed = 6;
		if (ownShip.blockedSector == 12) rudder = getRandomInt(1, 10) > 4 ? 3 : -3;
